...
def random_authen():
    sqn = random.choice(['sqn1', 'sqn2', 'sqn'])
    key = random.choice(['key1', 'key2', 'Key'])
    result = ia_authen(args.User, sqn, key)
    return result

def random_abac():
    action = random.choice(['read', 'delete','put', 'move', 'cut'])
    resource = random.choice(['resource1','resource2','resource3'])
    result = ac(args.User, resource, action)
    if result:
        return resource
    else:
        return None
...
if __name__ == '__main__':
    ...
    authority_pool=[]
    resource_pool=['resource1','resource2','resource20.1.1.1','resource20.1.2.4','resource20.1.3.7']
    resource_eid = {'resource1': '20.1.2.5', 'resource2': '20.1.2.6', 'resource20.1.1.1': '20.1.1.1', 'resource20.1.2.4': '20.1.2.4', 'resource20.1.3.7': '20.1.3.7'}
