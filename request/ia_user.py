...
# SUPI和K任意字符，sqn占8个字符，
def ia_register(SUPI='SUPI_test', sqn='sqn', K='Key', Role='admin', Cluster='ip707', Priviledge='8', ip=gateway_EID, port=ia_server_port):
    # SUPI=str(uuid.uuid3(uuid.NAMESPACE_DNS, SUPI))
    connection = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    connection.connect((ip, port))
    connection.send('register'.encode('utf-8'))
    ok = connection.recv(1024)
...
#     server.bind(('192.168.16.6', 12345))
#     print('UDP Server %s:%d is ready' % ('192.168.16.6', 12345))
#     while True:
#         re_authen_user, _ = server.recvfrom(1024)
#         re_authen_user = re_authen_user.decode('utf-8')
#         re_authen_user = re_authen_user.split('+++')
#         print('重新认证用户：', re_authen_user)
#         print('-' * 150)
#         for user_SUPI in re_authen_user:
#             aka_authen(user_SUPI, sqn[supi.index(user_SUPI)], sharedkey[supi.index(user_SUPI)], '192.168.200.4', 60000)
